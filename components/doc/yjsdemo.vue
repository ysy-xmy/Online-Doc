...
// 安全的粘贴处理函数(粘贴时会自动清除脚本、样式和不可见元素)(防止quill格式与yjs格式冲突)
const safePasteHandler = (quill, clipboardData) => {
    try {
        // 优先获取纯文本
        ...
        if (!text) {
            ...
            if (html) {
                // 使用 DOMParser 更安全地解析 HTML
                const parser = new DOMParser();
                const doc = parser.parseFromString(html, "text/html");

                // 移除所有脚本、样式和不可见元素
                const elementsToRemove = doc.querySelectorAll(
                    "script, style, [style*='display:none'], [style*='visibility:hidden']"
                );
...
const renderRemoteCursors = () => {
    ...
    console.log("渲染远程光标 - 所有状态:", allStates);
    ...
    Array.from(userStateMap.values()).forEach(({ clientID, state }) => {
        try {
            ...
            if (
                ...
                selectionElement.style.left = `${relativeLeft}px`;
                selectionElement.style.top = `${relativeTop}px`;
                ...
                selectionElement.style.border = `2px solid ${
                    user.color || "blue"
                }`;
...
// 防抖版本的渲染远程光标（编辑器滚动）
const debouncedRenderCursors = debounce(renderRemoteCursors, 2);

// 防抖版本的渲染远程光标
const debouncedRenderRemoteCursors = debounce(renderRemoteCursors, 25);
...
const initCollaborativeEditor = async () => {
    ...
    quill = new Quill(quillEditor.value, {
        ...
        modules: {
            ...
            clipboard: {
                matchVisual: false, // 禁用视觉匹配，避免复杂的 DOM 结构
                matchers: [], // 禁用所有默认的匹配器
            },
        },
    ...
    quill.root.addEventListener("paste", (event) => {
        ...
        // 使用安全的粘贴处理函数
        const text = safePasteHandler(quill, clipboardData);
    ...
    ytext.observe((event) => {
        ...
        if (event.changes.delta && event.changes.delta.length > 0) {
            try {
                ...
                console.warn("Yjs 文本变更处理出错:", error);
            }
        }
