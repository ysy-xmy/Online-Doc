<template>
    <div class="flex h-screen bg-base-100">
                ...
                <div class="flex-1 relative">
                    ...
                    >
                            ...
                            <div class="space-y-1">
                                ...
                                >
                                    <div class="flex items-center">
                                        <svg
                                            xmlns="http://www.w3.org/2000/svg"
                                            class="h-4 w-4 mr-2 text-base-content/50"
                                            fill="none"
                                            viewBox="0 0 24 24"
                                            stroke="currentColor"
                                        >
                        ...
                        <div
                            v-if="
                                searchKeyword.trim() &&
                                (searchSuggestions.workspaces.length > 0 ||
                        ...
                        >
                            ...
                            <div v-if="searchSuggestions.workspaces.length > 0">
                                <div
                                    class="px-3 py-2 text-xs text-base-content/50 bg-base-50"
                                >
                                    知识库
                                </div>
                                <div
                                    v-for="(
                                        workspace, index
                                    ) in searchSuggestions.workspaces.slice(
                                        0,
                                        3
                                    )"
                                    :key="workspace.id"
                                ...
                                >
                                    <div
                                        class="w-6 h-6 bg-primary/10 rounded flex items-center justify-center mr-3"
                                    >
                                        <span class="text-sm">{{
                                            workspace.icon || "📁"
                                        }}</span>
                            ...
                            <div v-if="searchSuggestions.documents.length > 0">
                                ...
                                <div
                                    v-for="(
                                        document, index
                                    ) in searchSuggestions.documents.slice(
                                        0,
                                        3
                                    )"
                                    :key="document.id"
                                ...
                                >
                                    <div
                                        class="w-6 h-6 bg-info/10 rounded flex items-center justify-center mr-3"
                                    >
                                        <span class="text-sm">{{
                        ...
                        <div
                            v-else-if="
                                searchKeyword.trim() && searchLoading === false
                            "
        ...
        <div v-if="showSearchModal" class="modal modal-open">
            <div
                class="modal-box w-11/12 max-w-4xl max-h-[80vh] overflow-hidden flex flex-col"
            >
                <!-- 弹窗头部 -->
                <div class="flex items-center justify-between mb-4">
                    <h3 class="font-bold text-lg">
                        搜索结果：{{ currentSearchKeyword }}
                    </h3>
                    <button
                ...
                <div class="mb-4 text-sm text-base-content/70">
                    找到 {{ totalResults }} 个结果
                    <span v-if="workspaceResults.length > 0"
                        >（{{ workspaceResults.length }} 个知识库</span
                    ><span
                        v-if="
                        ...
                        "
                        >，</span
                    ><span v-if="documentResults.length > 0"
                        >{{ documentResults.length }} 个文档</span
                    ><span
                        v-if="
                            workspaceResults.length > 0 ||
                            documentResults.length > 0
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                ...
                                >
                                    <div class="card-body p-3">
                                        <h5
                                            class="card-title text-sm flex items-center"
                                        >
                                        ...
                                        <div
                                            class="flex items-center justify-between mt-2 text-xs text-base-content/60"
                                        >
                                            <span>{{
                                ...
                                >
                                    <div class="card-body p-3">
                                        ...
                                        >
                                            <div class="flex-1">
                                                <h5
                                                    class="card-title text-sm flex items-center mb-1"
                                                >
                                                    <span
                                                    ...
                                                    {{ document.title }}
                                                    <div
                                                        class="badge badge-xs badge-outline ml-2"
                                                    >
                                                        {{
                                                            getDocumentTypeText(
                                                                document.type
                                                ...
                                                <p
                                                    ...
                                                    class="text-xs text-base-content/70 line-clamp-2 mb-2"
                                                >
                                                    {{ document.content }}
                                                </p>
                                                <div
                                                    class="flex items-center space-x-3 text-xs text-base-content/60"
                                                >
                                                    <span>{{
                                                        document.workspaceName
                                                    }}</span>
...
const selectItemByIndex = (index) => {
    if (index < searchHistory.value.length) {
        ...
        index <
        searchHistory.value.length + searchSuggestions.value.workspaces.length
    ) {
        // 选择知识库
        const workspaceIndex = index - searchHistory.value.length;
        navigateToWorkspace(
            searchSuggestions.value.workspaces[workspaceIndex].id
        );
    } else {
        // 选择文档
        const documentIndex =
