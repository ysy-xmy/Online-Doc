<template>
    <div class="bg-base-100 p-4 h-full flex flex-col">
        <!-- 错误提示 -->
        <div
            v-if="workspaceStore.error || documentStore.error"
        ...
        <div class="flex items-center justify-between mb-4">
            <div class="flex items-center space-x-2">
                ...
                >
                    <span class="text-2xl">{{
                        workspaceStore.currentWorkspace.icon || "📁"
                    }}</span>
        ...
        <div
            ...
            class="overflow-x-hidden flex-1 h-0"
        >
            <table class="table table-zebra table-pin-rows">
                ...
                <tbody>
                    ...
                    >
                        ...
                        <td
                            class="group-hover:text-base-content/80 transition-colors"
                        >
                            <div class="flex items-center space-x-2">
                                <div class="avatar avatar-xs">
                                    <div
                                        class="w-6 h-6 rounded-full bg-primary text-primary-content flex items-center justify-center text-xs"
                                    >
                                        {{
                                            doc.creator.nickname?.[0] ||
                                            doc.creator.username[0]
                                        }}
                                    </div>
                                </div>
                                <span>{{
                                    doc.creator.nickname || doc.creator.username
                                }}</span>
                            </div>
                        </td>
                        <td
                        ...
                        >
                            ...
                            >
                                <div class="avatar avatar-xs">
                                    <div
                                        class="w-6 h-6 rounded-full bg-secondary text-secondary-content flex items-center justify-center text-xs"
                                    >
                                        {{
                                            doc.lastModifier.nickname?.[0] ||
                                            doc.lastModifier.username[0]
                                        }}
                                    </div>
                                </div>
                                <span>{{
        ...
        <div
            v-else
            class="flex flex-col items-center justify-center min-h-[calc(100vh-200px)] text-base-content/60"
        >
            <svg
                xmlns="http://www.w3.org/2000/svg"
                class="h-48 w-48 mb-8 opacity-30"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
            >
            ...
            <p class="text-2xl mb-6 font-semibold text-base-content/70">
                暂无文档
            </p>
        ...
        >
            <div
                class="bg-base-100 rounded-xl shadow-xl w-full max-w-md p-8 relative"
            >
                <button
                ...
                <form @submit.prevent="onCreateDoc">
                    <div class="mb-4">
                        <label
                            class="block text-base-content font-semibold mb-1"
...
onMounted(async () => {
    ...
    if (workspaceId) {
        ...
        // 获取文档列表
        await documentStore.fetchDocumentsByWorkspace(workspaceId, {
            refresh: true,
        });
    }
});
